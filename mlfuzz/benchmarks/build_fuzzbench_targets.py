...
def main():
    ...
    # newer commit
    ex("cd /fuzzbench && git checkout 3234e151d70f6093272a9b4155e68ee57faf986c")
...
def build_curl():
    ...
    # new ossfuzz.sh has this hardcoded. maybe doesn't matter?
    # src = f"{base}/curl"
    src = f"/src/curl"
    work = f"{base}/b"
    out = os.environ["MOUNT_PATH"]
    ...
    ex(
        ...
        # newer commit
        f"git -C ./curl-fuzzer checkout 1fa81171ccf929ecb10f23256710b85b67f93e0d && "
        f"git clone  https://github.com/curl/curl.git {src} && "
        # f"git -C ./curl checkout a20f74a16ae1e89be170eeaa6059b37e513392a4"
        # newer commit
        f"git -C {src} checkout 2eebc58c4b8d68c98c8344381a9f6df4cca838fd"
    )

    ex(
        f"mkdir {work} && cd {base}/curl-fuzzer && chmod a+x ./ossfuzz.sh && ./ossfuzz.sh"
        # don't think this applies anymore since no install_curl.sh in ossfuzz.sh now
...
def build_zlib():
    ...
    ex(
        ...
        # newer commit
        f"cd {base} &&  git clone https://github.com/madler/zlib.git && \
            git -C zlib checkout da607da739fa6047df13e66a2af6b8bec7c2a498 "
    )
    ex("")
